...
async function viewBorough() {
  ...
  function budgetUI() {
    const b = payload.budget ?? {}; const rows = [["subsidy", "Trap subsidy", "Pays businesses per ton contained."], ["rd", "Trap R&D", "Lowers containment cost next round."], ["defense", "Slime defense", "Reduces slime damage this round."], ["reserve", "Reserve", "Unspent."]];
  ...
  shell("borough", `
    ...
    <div class="decision"><div class="eyebrow">This week's decision · ${iAmMinister ? "you are the mayor" : `${esc(minister?.display_name ?? "the mayor")} submits`}</div><h3>${esc(bcfg.title ?? "Set a containment target")}</h3>${bcfg.text ? `<p>${esc(bcfg.text)}</p>` : ""}
      ...
      <div class="preview"><div class="stat ecto"><div class="eyebrow">Ghosts released</div><div class="num" id="bev">…</div></div><div class="stat pos"><div class="eyebrow" id="bl2">Revenue</div><div class="num" id="brv">…</div></div><div class="stat haunt"><div class="eyebrow">Slime damage</div><div class="num" id="bhv">…</div></div></div>
